...
Scribe captures most decisions automatically from conversation. Use `/rune:capture` when Scribe missed something or you want to store context that isn't part of the current conversation:

### Security Policies

```
/rune:capture "All API keys must be stored in environment variables,
...
```
/rune:capture "We use the Repository pattern for data access.
...
```
/rune:capture "Database queries must use pagination. Maximum page size
...
```
/rune:capture "New developers should read docs/ARCHITECTURE.md first,
...
The index name is managed by the Vault admin and distributed automatically at startup.
...
```
/rune:capture "$(cat docs/ARCHITECTURE.md)"
```
...
```
/rune:capture "Updated database decision: We migrated from PostgreSQL
...
**Solution**: Use `/rune:capture` as a manual override:
```
/rune:capture "Previous decision: [repeat the important context]"
```
...
**Solution**: State the updated decision in conversation (Scribe will capture it), or use `/rune:capture` to force-store:
```
/rune:capture "Updated: [new information that supersedes old decision]"
```
...
**Manual Override (`/rune:capture`)**: Force-store context that Scribe missed — use sparingly.
