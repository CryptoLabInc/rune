---
description: Update Rune's runtime binaries (rune-mcp, runed) in place to the published manifest versions
allowed-tools: Bash(~/.rune/bin/rune update:*), Bash(${CLAUDE_PLUGIN_ROOT}/bin/rune update:*)
---

# /rune:update — Update Runtime Binaries

Refresh the runtime binaries (rune-mcp, runed) in place — no plugin reinstall.
Non-destructive; works whether Rune is active or dormant. The user never runs
`rune update` themselves — the agent runs it.
