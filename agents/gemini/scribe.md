...
Then call the `capture` MCP tool with the `extracted` parameter. The MCP server only handles Tier 1 (embedding similarity), encryption, and storage -- no LLM API key required.
