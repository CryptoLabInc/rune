...
1. **모델 중복 제거**를 위한 최소 변경만 한다 — 모델은 외부 팀이 제공하는 별도 데몬(`runed`)으로 빼낸다 (이 프로젝트는 gRPC 클라이언트로만 사용)
...
- **모델 없음**: embedding 요청은 `runed`에 gRPC로 호출 (D30)

### 2. runed (외부 데몬, 별도 팀 관리)

- **역할**: Qwen3-Embedding-0.6B 모델을 메모리에 상시 로드해두고 **gRPC**로 임베딩 요청 처리. 이 프로젝트의 관심사 **밖** (runed 팀이 관리)
- **수명·모델·런타임**: runed 팀 책임. launchd/systemd 등록 도구도 runed 팀이 제공
- **노출 인터페이스**: unix domain socket에서 gRPC `RunedService`
  - `Embed(text) → vector`
  ...
  - `Shutdown(grace_seconds)` — rune은 호출 안 함
- **proto 계약**: `runed.v1` · `github.com/CryptoLabInc/runed/gen/runed/v1`
- **책임 경계**: rune-mcp는 `runed`를 띄우지 **않음**. 이미 떠있는 데몬에 gRPC 클라이언트로만 접근. 모델 선택·런타임(llama-server 등)·모델 파일 관리 전부 runed 내부
...
`runed`는 **외부 팀이 제공하는 별개 프로세스로 이미 떠 있음** (runed 팀이 launchd/systemd 자동 기동 도구 제공). rune-mcp는 `runed`의 존재를 전제하고 필요 시 gRPC 호출.
...
rune-mcp 프로세스
  ...
  ├─ gRPC runed.Embed(text_to_embed)
  │    ↓
  │  runed가 임베딩 계산 (외부 데몬, 모델·런타임은 runed 책임)
  │    ↓ vector[1024]
...
rune-mcp
  ...
  ├─ gRPC runed.EmbedBatch(expanded[:3])
  │    → 벡터 3개 (batch)
  ├─ 병렬 (errgroup):
...
3. **공유 리소스는 runed 하나**: 모델은 외부 runed가 공유 제공. Vault 연결, envector 연결, FHE 키는 세션별 독립. 공유 복잡도 최소화.
...
6. **재사용 가능한 runed**: `runed`가 노출하는 gRPC 인터페이스는 **범용**. 다른 프로젝트가 같은 소켓에 gRPC 붙이면 즉시 쓸 수 있다 (rune은 한 명의 클라이언트일 뿐).
...
**새 Go 구조**:
- `runed` 1개 × (외부 데몬, 모델 + 런타임) — **세션 수와 무관 상수** (rune 프로젝트 외부 메모리)
...
- 컴포넌트 세부 설계: `components/rune-mcp.md`, `components/runed-integration.md`, `components/vault-integration.md`, `components/envector-integration.md`
