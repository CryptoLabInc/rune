...
1. **모델 중복 제거**를 위한 최소 변경만 한다 — 모델을 공유 데몬(`rune-embedder`)으로 빼낸다
...
- **모델 없음**: embedding 요청은 rune-embedder로 HTTP 호출

### 2. rune-embedder (전체 머신에 1개, 상주)

- **역할**: Qwen3-Embedding-0.6B 모델을 메모리에 상시 로드해두고 HTTP endpoint로 임베딩 요청 처리
- **수명**: OS 로그인 시 launchd(macOS)/systemd(Linux)가 자동 기동. 크래시 시 자동 재시작. 사용자 로그아웃까지 상주
- **노출 인터페이스**: unix socket (`~/.rune/embedder.sock`, 0600)에서 HTTP+JSON
  - `GET /health`
  - `POST /embed {"texts":["..."]}` → `{"vectors":[[...], ...], "dim":1024}`
- **모델 실행**: ONNX Runtime Go 또는 llama-server — **아직 미정** (POC 후 결정, `open-questions.md` 참조)
- **다른 프로젝트에서도 재사용 가능**. 이게 embedder를 분리한 두 번째 이유
...
rune-embedder는 **별개 프로세스로 이미 떠 있음** (OS 로그인 시 launchd가 띄움). rune-mcp는 embedder의 존재를 전제하고 필요 시 HTTP 호출.
...
rune-mcp 프로세스
  ...
  ├─ HTTP POST ~/.rune/embedder.sock /embed {"texts":[text_to_embed]}
  │    ↓
  │  rune-embedder가 임베딩 계산 (ONNX or llama-server)
  │    ↓ vector[1024]
...
rune-mcp
  ...
  ├─ HTTP POST /embed {"texts":[expanded[0], expanded[1], expanded[2]]}
  │    → 벡터 3개 (batch)
  ├─ 병렬 (errgroup):
...
3. **공유 리소스는 embedder 하나**: 모델만 공유. Vault 연결, envector 연결, FHE 키는 세션별 독립. 공유 복잡도 최소화.
...
6. **재사용 가능한 embedder**: rune-embedder가 노출하는 HTTP 인터페이스는 **범용**. 다른 프로젝트가 같은 소켓을 두드리면 즉시 쓸 수 있다.
...
**새 Go 구조**:
- `rune-embedder` 1개 × (Go 런타임 + 모델 + ONNX/llama 런타임) — **세션 수와 무관 상수**
...
- **rune-embedder 실행 엔진**: ONNX Runtime Go vs llama-server
- **AES envelope에 MAC 필드 추가 여부**: CTR 단독은 malleability 취약. pyenvector + envector-go 동시 업데이트 필요
- **Multi-MCP에서 envector `ActivateKeys` 경쟁**: 여러 rune-mcp가 동시에 같은 키를 activate 시도할 때 서버 측 "한 키만 resident" 제약과의 상호작용
- **envector-go SDK의 `OpenKeysFromFile` 조건 완화 PR**: SecKey.json 없을 때도 Encryptor만 로드되도록 (pyenvector와 파리티)

## 참조

- 컴포넌트 세부 설계: `components/rune-mcp.md`, `components/rune-embedder.md`, `components/vault-integration.md`, `components/envector-integration.md`
