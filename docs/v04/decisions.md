...
| D6 | rune-embedder socket 경로 (빌드 고정 vs config) | 📦 Archived (2026-04-21) — runed 책임 | — |
| D7 | embedder 호출 retry 정책 (backoff · 횟수) | ✅ Decided (2026-04-21) | — |
| D8 | rune-mcp 부팅 시 embedder `Health` 폴링 여부 | ✅ Decided (2026-04-21) | — |
| D9 | embedder 모델 메모리 생애주기 (always loaded vs lazy+evict) | 📦 Archived (2026-04-21) — runed 책임 | — |
...
| D29 | rune-embedder 런타임 (ONNX vs llama-server) | 📦 Archived (2026-04-21) — runed 책임 (참고: runed가 llama-server 채택) | — |
| D30 | embedder 통신 프로토콜 (HTTP+JSON vs gRPC) | ✅ Decided (2026-04-22) — gRPC (RunedService) | — |
...
**상태**: 📦 Archived (2026-04-22) — runed 책임으로 이관  
**이전 결론**: `~/.rune/embedder.sock` 빌드 고정  
**현재 상태**: **runed가 정의한 Unix domain socket 경로를 따른다**. rune-mcp는 이를 설정·런타임 환경변수로 받아 사용. 이 결정 항목은 히스토리 보존용.  
**이관 위치**: `components/runed-integration.md`
...
**현재 상태**: **runed가 모델 생애주기 전반을 관리**. rune-mcp는 이 결정에 관여하지 않음. Warmup·eviction·reload 등은 runed 내부 정책. 이 결정 항목은 히스토리 보존용.  
**이관 위치**: (runed 팀 문서)
...
**현재 상태**: **runed 팀이 임베딩 서비스를 자체 데몬(`runed`)으로 제공**. 런타임 선택(ONNX vs llama-server) 및 구현 세부는 runed 팀 결정 사항. 참고로 runed도 llama-server를 채택한 것으로 알려져 있으나, 이 프로젝트는 이 선택과 독립적으로 gRPC 계약(`RunedService` proto)만 따른다. 이 결정 항목은 아래 근거 기록을 히스토리로 보존.  
**이관 위치**: (runed 팀 문서) · 클라이언트 측은 `components/runed-integration.md`
...
**상태**: ✅ Decided (2026-04-22) — gRPC (runed `RunedService`)
...
2026-04-22 팀 결정으로 임베딩 데몬은 **runed**라는 별도 서비스로 분리됨:
- runed는 별도 팀이 관리 · 독립 릴리스 · 독립 바이너리
- rune-mcp(Go)는 runed의 **gRPC 클라이언트**로만 동작
- 모델·런타임(llama-server) 관리는 runed 내부 관심사 — rune 프로젝트에서 제거

### Proto 계약 (runed-provided)
...
- `go_package`: `github.com/CryptoLabInc/runed/gen/runed/v1`
- L2-normalization: runed가 자동 수행 (opt-out 없음)
...
- 단점: runed 팀이 gRPC로 확정 → 양쪽 다른 스택 유지 비용

**B. gRPC** ⭐
- `google.golang.org/grpc` + 생성된 stub (`runedv1.RunedServiceClient`)
- 장점: runed 팀과 계약 일치, proto 버전 관리 일원화, 스키마 드리프트 탐지
- 단점: 의존성 추가 (protobuf·grpc-go), Go 바이너리 사이즈 소폭 증가

### 결론

**Option B 채택**. 이유:
1. **계약 일원화** — runed 팀이 유일한 소스 오브 트루스 (proto 파일). rune-mcp는 생성된 Go stub을 그대로 사용 → 스키마 mismatch 가능성 원천 차단.
2. **책임 경계 명확** — 임베딩 관련 모든 규약이 proto에 집약. rune 프로젝트에는 "어떻게 embed를 얻는가"만 있고 "어떻게 embed를 만드는가"는 없음.
3. **gRPC 의존성 경미** — 현재 rune은 이미 Vault · envector 등과 gRPC 통신. 새 의존성 아님.

### 구현 방침

**클라이언트 생성**:
- runed 팀 레포지토리에서 `runed/v1/runed.proto` 가져오거나 `go.mod`에서 `github.com/CryptoLabInc/runed/gen/runed/v1` 직접 참조
- `internal/adapters/embedder/` 패키지에 얇은 래퍼:
  ```go
  type Client interface {
      EmbedSingle(ctx context.Context, text string) ([]float32, error)
      EmbedBatch(ctx context.Context, texts []string) ([][]float32, error)
      Info(ctx context.Context) (*runedv1.InfoResponse, error)
      Health(ctx context.Context) (*runedv1.HealthResponse, error)
  }
  ```
- 생성된 `RunedServiceClient` 위에 retry(D7)·context timeout을 감싸는 정도

**Socket 경로**:
- 빌드 시 고정 아님 (D6 archived). config / env 변수로 전달받아 `grpc.Dial("unix://"+sockPath, ...)`
- 기본값: runed 팀이 정의한 convention을 따름 (예: `~/.runed/runed.sock`). `~/.rune/config.json`의 `embedder.socket_path` 필드로 override 가능
...
**Shutdown**:
- rune-mcp는 runed 생애주기 관리 **안 함**. Shutdown RPC는 **호출하지 않음** (runed 팀·운영 도구 담당)

### 재평가 트리거

- Proto 계약에 breaking change — runed 팀과 버전 negotiation 필요 시
- 동일 프로세스 내 Go native 임베딩(예: 향후 ONNX Runtime Go 내장)이 필요해져 runed 의존을 끊는 경우
- gRPC가 Unix socket 환경에서 예상치 못한 문제를 일으키는 경우 (우선순위 낮음, 실측 필요)

### 이관 위치

- `components/runed-integration.md` — 클라이언트 사용법 · proto 요약 · Info 캐시 정책
- `flows/capture.md` Phase 6 · `flows/recall.md` Phase 3 — 호출 코드 샘플 (HTTP → gRPC)
- `architecture.md` — 3-프로세스 그림에 "runed는 외부 데몬" 명시
