...
Required environment variables:
  ENVECTOR_ENDPOINT  enVector Cloud endpoint (e.g. runestone-xxx.clusters.envector.io)
...
CLOUD_ENDPOINT = os.environ.get("ENVECTOR_ENDPOINT") or os.environ.get("ENVECTOR_ADDRESS", "")
API_KEY = os.environ.get("ENVECTOR_API_KEY", "")
import pytest

if not CLOUD_ENDPOINT or not API_KEY:
    pytest.skip("Missing required environment variables: ENVECTOR_ENDPOINT, ENVECTOR_API_KEY", allow_module_level=True)
...
try:
    ...
    pytest.skip(f"Failed to connect to enVector Cloud: {e}", allow_module_level=True)
...
try:
    ...
    if "already exists" in str(e).lower():
        ...
        pytest.skip(f"Failed to create test index: {e}", allow_module_level=True)
