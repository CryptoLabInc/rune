...
>
> **[비교 노트]** 2026-05-05 리포트(pyenvector 1.4.0)와의 수치 차이는 **pyenvector 버전 차이**와 **엔드포인트 변경** 두 가지 요인이 복합되어 있어 버전 단독 영향으로 해석하면 안 됨.
...
### recall이 1.4.0 대비 ~1.4x 느려짐 — 엔드포인트 변경 영향
...
`score`(+90ms)와 `vault_topk`(+77ms)에서 주로 증가. 엔드포인트가 IP 직접 접속에서 도메인 기반 클라우드로 변경되어 DNS 조회 + TLS handshake 오버헤드가 추가된 것으로 추정.
